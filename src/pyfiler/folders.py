...
def _direct_child_name(name):
    if not isinstance(name, str) or not name.strip() or name in {".", ".."}: raise InvalidPathError("name must be a non-empty direct child name")
    if Path(name).name != name or Path(name).is_absolute(): raise InvalidPathError("name must not contain directory separators")


def _temporary_destination(parent, prefix):
    handle=tempfile.NamedTemporaryFile(dir=parent,prefix=prefix,suffix=".pyfiler-stage",delete=False)
    candidate=Path(handle.name); handle.close(); candidate.unlink(missing_ok=True); return candidate


def _copy_tree_staged(source,parent,name):
    stage=_temporary_destination(parent,f".{name}.")
    try: shutil.copytree(source,stage); return stage
    except Exception:
...
def create_folder(name,contents=None,trajectory=None):
    ...
    sources=[]; target=path.resolve()
    for child in children:
        source=to_path(child); source_resolved=source.resolve()
    ...
    try:
        ...
        stage=_temporary_destination(path.parent,f".{path.name}."); stage.mkdir()
    ...
    except OSError as exc: raise FolderCreateError(str(path)) from exc
    finally:
...
def _validate_children(folder,children):
    ...
    for name in children:
        ...
        seen.add(name); target=folder/name
...
def delete_folder(path,recursive=False):
    if not isinstance(recursive,bool): raise TypeError("recursive must be a boolean")
...
def copy_folder(source,destination,overwrite=False):
    _validate_overwrite(overwrite); src,dst=ensure_folder(source),to_path(destination); sr,dr=src.resolve(),dst.resolve()
    ...
    stage=backup=None
    try:
        dst.parent.mkdir(parents=True,exist_ok=True); stage=_copy_tree_staged(src,dst.parent,dst.name)
        if dst.exists():
            if not overwrite: raise DestinationExistsError(str(dst))
            backup=_temporary_destination(dst.parent,f".{dst.name}."); dst.rename(backup)
        try: stage.rename(dst); stage=None
        except OSError:
            if backup is not None and backup.exists() and not dst.exists(): backup.rename(dst)
            raise
        if backup is not None: shutil.rmtree(backup); backup=None
...
def move_folder(source,destination,overwrite=False):
    ...
    try:
        ...
        try: same_device=src.stat().st_dev==dst.parent.stat().st_dev
        except OSError: same_device=False
        if same_device and not dst.exists():
            os.replace(src,dst); return dst
        stage=_copy_tree_staged(src,dst.parent,dst.name)
        if dst.exists(): backup=_temporary_destination(dst.parent,f".{dst.name}."); dst.rename(backup)
        try: stage.rename(dst); stage=None
        except OSError:
            if backup is not None and backup.exists() and not dst.exists(): backup.rename(dst)
            raise
        if backup is not None: shutil.rmtree(backup); backup=None
        shutil.rmtree(src); return dst
...
def rename_folder(path,new_name):
    src=ensure_folder(path); _direct_child_name(new_name); dst=src.with_name(new_name)

...
def create_folder(name,contents=None,trajectory=None):
    ...
    stage=None
    try:
        path.parent.mkdir(parents=True,exist_ok=True); stage=_temporary_destination(path.parent,f".{path.name}."); stage.mkdir()
...
def copy_folder(source,destination,overwrite=False):
    ...
    try:
        ...
        if dst.exists():
            ...
            backup=_temporary_destination(dst.parent,f".{dst.name}."); os.replace(dst,backup)
        try: os.replace(stage,dst); stage=None
        ...
        if backup is not None and backup.exists() and not dst.exists(): _restore_backup(backup,dst)
    return dst
...
def move_folder(source,destination,overwrite=False):
    ...
    try:
        if same_device:
            if overwrite and dst.exists(): backup=_temporary_destination(dst.parent,f".{dst.name}."); os.replace(dst,backup)

...
def create_folder(name, contents=None, trajectory=None):
    ...
    if contents is not None and not isinstance(contents, (list, tuple)):
        raise ContentTypeError("contents must be a list or tuple of file/folder paths.")
...
def move_into(child, parent):
    ...
    if destination.exists():
        raise DestinationExistsError(str(destination))
    if source.is_dir():
        try: destination_folder.relative_to(source)
        except ValueError: pass
        else: raise RecursiveOperationError(f"Cannot move {source} into its own descendant {destination_folder}.")
    try:
...
def clear_folder(path):
    ...
    return remove_from_folder(folder, children)
...
def move_folder(source, destination, overwrite=False):
    src, dst = ensure_folder(source), to_path(destination)
    src_resolved, dst_resolved = src.resolve(), dst.resolve()
    if src_resolved == dst_resolved: raise SourceEqualsDestinationError(str(src))

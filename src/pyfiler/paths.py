...
def _lexical(path): return Path(os.path.normpath(os.path.expanduser(str(_path(path)))))
def absolute_path(path):
    p=_lexical(path)
    return str(p if p.is_absolute() else Path(os.path.abspath(str(p))))
def relative_path(path,start=None):
    target=Path(absolute_path(path)); base=Path(absolute_path(start if start is not None else Path.cwd()))
    try: return os.path.relpath(str(target),str(base))
    except ValueError as exc: raise ValueError(f"Paths cannot be made relative: {target!s}, {base!s}") from exc
def parent_path(path): return str(_lexical(path).parent)
...
def join_paths(*parts):
    ...
    for part in parts[1:]:
        ...
        result/=component
    return str(result)
def normalize_path(path): return str(_lexical(path))
...
def with_extension(path,extension):
    ...
    p=_path(path)
    # pathlib treats '.profile' as a stem with no suffix; preserve that name
    # when adding an extension rather than turning it into '.bak'.
    if p.name.startswith(".") and p.name.count(".")==1:
        return str(p.with_name(p.name+extension))
    return str(p.with_suffix(extension))

def is_root(path):
    p=_lexical(path); return p.parent==p
...
def relative_to(path,base):
    target=Path(absolute_path(path)); root=Path(absolute_path(base))
...
def add_suffix(path,suffix):
    ...
    p=_path(path); return str(p.with_name(p.stem+suffix+p.suffix))

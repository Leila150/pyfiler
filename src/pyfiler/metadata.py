...
def _stat(path):
    ...
    try:
        ...
        raise PathNotFoundError(str(p)) from exc
    except OSError as exc:
        raise MetadataUnavailableError(str(p)) from exc
...
def extension_of(path):
    p, st = _stat(path)
    return p.suffix if stat.S_ISREG(st.st_mode) else ""
...
def metadata(path):
    ...
    try:
        ...
        isfolder = stat.S_ISDIR(st.st_mode)
...
def path_kind(path):
    ...
    if stat.S_ISDIR(st.st_mode): return "folder"
    return "other"

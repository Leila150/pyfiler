...
def metadata(path):
    p,st=_stat(path); isfile=stat.S_ISREG(st.st_mode); isfolder=stat.S_ISDIR(st.st_mode)
    try: empty=(next(iter(p.iterdir()),None) is None) if isfolder else (st.st_size==0 if isfile else False)
    except OSError as exc: raise MetadataUnavailableError(str(p)) from exc
    return {"name":p.name,"path":str(p.resolve(strict=False)),"type":"file" if isfile else "folder" if isfolder else "other","size":st.st_size,"extension":p.suffix if isfile else "","created":st.st_ctime,"modified":st.st_mtime,"accessed":st.st_atime,"is_file":isfile,"is_folder":isfolder,"is_empty":empty}
...
def path_kind(path):
    ...
    return "file" if stat.S_ISREG(st.st_mode) else "folder" if stat.S_ISDIR(st.st_mode) else "other"

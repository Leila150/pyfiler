...
def _walk(path,pattern="*"):
    ...
    try: yield from root.rglob(pattern)
    except OSError as exc: raise SearchError(f"Unable to search {root}: {exc}") from exc
def find_paths(path,name="*"): return _sorted_paths(_walk(path,_validate_name(name)))
def find_files(path,name="*"):
    results=[]
    for p in find_paths(path,name):
        try:
    ...
    try:
        ...
        with path.open("r",encoding=encoding,newline="") as handle:
            for line in handle:
                ...
                yield line
        final=path.stat()
        if (initial.st_dev,initial.st_ino,initial.st_size,initial.st_mtime_ns)!=(final.st_dev,final.st_ino,final.st_size,final.st_mtime_ns): raise SearchError(f"File changed during search: {path}")
